...
const PIZZAS_ESPECIAIS = [
    ...
    {
        id: 23, name: "Pizza Bella Vista", category: "Pizzas Especiais",
        desc: "Muçarela, lombo canadense, presunto, bacon, cebola, azeitona preta e cream cheese.",
        img: "assets/BellaVista.webp",
...
const loadCart = () => {
    try {
        const saved = localStorage.getItem('bellavista-cart');
...
const saveCart = () => {
    try { localStorage.setItem('bellavista-cart', JSON.stringify(cart)); } catch (e) { }
};
...
const renderPizzaCard = (pizza) => {
    ...
    card.className = "flex items-center gap-4 p-4 bg-white border border-gray-100 shadow-sm hover:shadow-md hover:border-orange-200 transition-[transform,box-shadow,border-color] duration-300 cursor-pointer group relative z-10";
    ...
    card.innerHTML = `
        <div class="flex-1 min-w-0 pr-2">
            <h3 class="text-base font-bold text-gray-900 mb-1 font-serif uppercase truncate">${pizza.name}</h3>
            <p class="text-xs text-gray-500 line-clamp-2 mb-4 leading-relaxed">${pizza.desc}</p>
            <div class="flex items-center gap-2">
                <span class="text-[11px] font-bold text-gray-400 uppercase tracking-tighter">A partir de</span>
        ...
        <div class="w-24 h-24 md:w-32 md:h-32 flex-shrink-0 overflow-hidden rounded-2xl">
            <img src="${pizza.img}" alt="${pizza.name}" class="w-full h-full object-cover transition-transform group-hover:scale-110 duration-500" 
                 onerror="this.src='assets/logobellavista.webp'">
        </div>
    `;
    return card;
};
...
const renderDrinkCard = (drink) => {
    ...
    card.innerHTML = `
        <div class="flex-1 min-w-0 pr-2">
            <h3 class="text-base font-bold text-gray-900 mb-1 font-serif uppercase truncate">${drink.name}</h3>
            <p class="text-xs text-gray-500 line-clamp-2 mb-4 leading-relaxed">${drink.desc}</p>
            <div class="flex items-center gap-2">
                ${hasVariations ? '<span class="text-[11px] font-bold text-gray-400 uppercase tracking-tighter">A partir de</span>' : ''}
                <span class="text-base font-bold text-orange-600">${formatCurrency(minPrice)}</span>
            </div>
        </div>
        <div class="w-24 h-24 md:w-32 md:h-32 flex-shrink-0 overflow-hidden rounded-2xl">
...
function renderHalfFlavors() {
    ...
    grid.innerHTML = ALL_PIZZA_FLAVORS.map(pizza => `
        <div onclick="selectFlavor(${pizza.id})" id="flavor-card-${pizza.id}" class="flavor-card relative bg-white border border-gray-200 rounded-xl overflow-hidden cursor-pointer transition-all hover:bg-orange-50 hover:border-orange-200 flex flex-col group">
            <div class="relative w-full aspect-[4/3] bg-gray-100 overflow-hidden">
                <img src="${pizza.img}" alt="${pizza.name}" class="w-full h-full object-cover transition-transform group-hover:scale-105" 
                     onerror="this.src='assets/logobellavista.webp'">
            </div>
            <div class="p-3 text-left flex-1 flex flex-col">
                <span class="font-bold text-gray-900 text-sm mb-1 uppercase">${pizza.name}</span>
                <span class="text-xs text-gray-500 leading-tight">${pizza.desc}</span>
            </div>
            <div class="selection-badge selection-badge-1 absolute bottom-0 left-0 right-0 py-1 bg-red-600 text-white text-center text-[10px] font-bold uppercase hidden">1º SABOR</div>
            <div class="selection-badge selection-badge-2 absolute bottom-0 left-0 right-0 py-1 bg-blue-600 text-white text-center text-[10px] font-bold uppercase hidden">2º SABOR</div>
        </div>
    `).join('');
...
function syncHalfVisuals() {
    document.querySelectorAll('.flavor-card').forEach(c => {
        c.classList.remove('border-red-600', 'border-blue-600', 'shadow-md');
        c.classList.add('border-gray-200');
        c.querySelector('.selection-badge-1').classList.add('hidden');
        c.querySelector('.selection-badge-2').classList.add('hidden');
    });
    ...
    if (halfPizzaState.flavor1) {
        ...
        if (el) {
            ...
            el.classList.add('border-red-600', 'shadow-md');
            el.querySelector('.selection-badge-1').classList.remove('hidden');
        }
    }
    if (halfPizzaState.flavor2) {
        ...
        if (el) {
            ...
            el.querySelector('.selection-badge-2').classList.remove('hidden');
        }
    }
}
...
function setHalfSize(s) {
    ...
    document.querySelectorAll('.half-size-btn').forEach(b => {
        b.classList.remove('bg-[var(--brand-red)]', 'border-[var(--brand-red)]', 'shadow-md');
        b.classList.add('bg-white', 'border-gray-200');
        b.querySelector('span:first-child').classList.remove('text-white');
        b.querySelector('span:first-child').classList.add('text-gray-800');
        b.querySelector('span:last-child').classList.remove('text-white', 'opacity-90');
    ...
    if (btn) {
        ...
        btn.classList.add('bg-[var(--brand-red)]', 'border-[var(--brand-red)]', 'shadow-md');
        btn.querySelector('span:first-child').classList.remove('text-gray-800');
        ...
        btn.querySelector('span:last-child').classList.add('text-white', 'opacity-90');
    }
...
function openProductModal(item) {
    ...
    modalImg.onerror = () => {
        modalImg.src = 'assets/logobellavista.webp';
    };
...
const renderHalfCard = () => {
    const card = document.createElement('div');
    card.className = "bg-orange-600 p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 text-white cursor-pointer hover:bg-orange-700 transition-all shadow-md relative overflow-hidden group col-span-1 md:col-span-2";
    card.onclick = openHalfModal;

    card.innerHTML = `
        <div class="absolute top-4 right-4 p-2 opacity-90 transition-transform group-hover:scale-110">
            <i data-lucide="pizza" class="w-6 h-6 md:w-8 md:h-8 text-white"></i>
        </div>
        <div class="flex-1 relative z-10 text-center md:text-left pt-6 md:pt-0 pl-0 md:pl-2">
            <span class="text-[12px] md:text-sm font-bold uppercase tracking-widest mb-1 inline-block text-white opacity-100">FUNCIONALIDADE EXCLUSIVA</span>
            <h3 class="text-2xl md:text-3xl font-bold mb-3 font-serif tracking-tight text-white uppercase">CRIAR PIZZA MEIO A MEIO</h3>
            <p class="text-xs md:text-sm md:text-base opacity-100 max-w-xl mb-6 leading-relaxed">Não consegue decidir? Escolha dois dos seus sabores favoritos em uma única pizza Média ou Grande.</p>
            <div class="flex flex-wrap justify-center md:justify-start gap-4">
                <span class="flex items-center gap-2 border border-white px-4 py-1.5 rounded-full text-xs font-bold italic">
                    <i data-lucide="check-circle" class="w-4 h-4 text-white"></i> Média 6 Fatias
                </span>
                <span class="flex items-center gap-2 border border-white px-4 py-1.5 rounded-full text-xs font-bold italic">
                    <i data-lucide="check-circle" class="w-4 h-4 text-white"></i> Grande 10 Fatias
                </span>
            </div>
        </div>
        <div class="relative z-10 bg-[#f0dfc1] text-orange-700 px-8 py-6 md:px-10 md:py-8 flex flex-col items-center justify-center gap-2 transition-transform shadow-lg group-hover:scale-105 mt-4 md:mt-0 mx-auto md:ml-auto md:mr-4 text-center shrink-0">
            <span class="text-xs md:text-sm font-medium uppercase tracking-widest leading-tight">Monte Agora</span>
            <i data-lucide="arrow-right" class="w-6 h-6 text-orange-700"></i>
        </div>
    `;
    return card;
};
...
const renderMenu = () => {
    ...
    CATEGORIES.forEach(cat => {
        ...
        title.className = "text-xl md:text-2xl font-bold text-gray-900 mb-6 pb-2 border-b border-orange-500 font-serif w-full block";
...
function renderCheckoutForm() {
    ...
    form.innerHTML = `
        ...
        <div>
            <label class="block text-xs font-bold text-gray-500 uppercase mb-3">Forma de Pagamento</label>
            <div class="grid grid-cols-3 gap-3 sm:gap-4">
                <div class="flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all cursor-pointer aspect-[4/3] ${checkoutData.paymentMethod === 'Pix' ? 'border-[var(--brand-red)] text-[var(--brand-red)] bg-orange-50' : 'border-gray-200 text-gray-500 hover:border-orange-200'}" onclick="setPaymentMethod('Pix')">
                    <i data-lucide="qr-code" class="w-6 h-6 md:w-8 md:h-8 mb-2"></i>
                    <span class="text-[10px] md:text-xs font-bold uppercase tracking-wide">Pix</span>
                </div>
                <div class="flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all cursor-pointer aspect-[4/3] ${checkoutData.paymentMethod === 'Dinheiro' ? 'border-[var(--brand-red)] text-[var(--brand-red)] bg-orange-50' : 'border-gray-200 text-gray-500 hover:border-orange-200'}" onclick="setPaymentMethod('Dinheiro')">
                    <i data-lucide="banknote" class="w-6 h-6 md:w-8 md:h-8 mb-2"></i>
                    <span class="text-[10px] md:text-xs font-bold uppercase tracking-wide">Dinheiro</span>
                </div>
                <div class="flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all cursor-pointer aspect-[4/3] ${checkoutData.paymentMethod === 'Cartão' ? 'border-[var(--brand-red)] text-[var(--brand-red)] bg-orange-50' : 'border-gray-200 text-gray-500 hover:border-orange-200'}" onclick="setPaymentMethod('Cartão')">
                    <i data-lucide="credit-card" class="w-6 h-6 md:w-8 md:h-8 mb-2"></i>
                    <span class="text-[10px] md:text-xs font-bold uppercase tracking-wide">Cartão</span>
                </div>
            </div>
        </div>
...
function sendToWhatsApp() {
    ...
    msg += `*--------------------------*\n`;
    msg += `_Bella Vista agradece a preferência!_ 🔥`;
...
window.onload = function () {
    ...
    if (ctaBtn) {
        ...
        // Timeline da animação (agora toca automaticamente)
